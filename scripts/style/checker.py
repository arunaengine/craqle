...
from content import comment_issues, header_issues
from model import Issue
from tokens import Comment, Name, Scan, rust_names, rust_tokens, terms


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("root", nargs="?", type=Path, default=default_root(Path(__file__)))
    parser.add_argument("--rules", type=Path, default=Path(__file__).with_name("rules.json"))
    parser.add_argument("--json", action="store_true")
    parser.add_argument("--inventory", action="store_true",
                        help="print JSON findings without turning findings into a failure")
    ...
    return 0 if args.inventory else int(bool(issues))


def default_root(script):
    """Resolves the repository root from the nested style entry point."""
...
def load_rules(path):
    ...
    for name in rules.get("exception_files", []):
        rules["exceptions"].extend(json.loads(path.with_name(name).read_text()))
    needed = {"maintained_extensions", "source_extensions", "header_extensions",
              "max_comment_lines",
              "required_roots", "small_domains",
    ...
    if not modes <= {"single_type", "function_list", "static_list"}:
...
def file_issues(root, file, rules):
    ...
    suffix = file.name.lower() if file.name == ".gitignore" else file.suffix.lower()
    if skipped(path, rules, "content"):
        return []
    if file.is_symlink():
    ...
    issues = header_issues(path, source, suffix, rules)
    if suffix not in rules["source_extensions"]:
        return issues
    ...
    issues.extend(Issue(path, line, "syntax", "source", "source", detail)
    ...
    issues.extend(comment_issues(path, comments, source, rules["max_comment_lines"]))
    return issues
...
def path_suffix(path):
    """Returns the maintained extension for regular and dot files."""
    name = Path(path).name
    return name.lower() if name == ".gitignore" else Path(name).suffix.lower()
...
def layout_issues(paths, rules):
    ...
    maintained = [path for path in visible if path_suffix(path) in extensions]
    issues = [Issue(path, 1, "unsupported_asset", Path(path).name, "asset",
                    "owned file type is not classified by style rules")
              for path in visible if path_suffix(path) not in extensions]
    ...
    for folder, members in sorted(folders.items()):
        ...
        for path in direct:
            ...
            if "_" in stem or "-" in stem:
                ...
                if prefix not in generic and not prefix.isdigit():
