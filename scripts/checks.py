#!/usr/bin/env python3
"""Run serial checks with exact source, fixture, lock, and log evidence."""
...
def identity(repo):
    ...
    for path in sorted((repo / "tests" / "fixtures").rglob("*")):
        if path.is_file():
            fixtures[path.relative_to(repo).as_posix()] = file_hash(path)
    return {
...
def file_hash(path):
    """Return one file's SHA-256 digest without loading it whole."""
    digest = hashlib.sha256()
    with path.open("rb") as source:
        for chunk in iter(lambda: source.read(1024 * 1024), b""):
            digest.update(chunk)
        ...
        with (output / "CHECKS.csv").open("x", newline="") as table:
            ...
            for case in cases:
                ...
                try:
                    ...
                    argv = [arg.replace("{output}", str(output)) for arg in case["argv"]]
                    with path.open("x") as log:
                        log.write(json.dumps({"argv": argv, "source": before,
