//! Compares incremental and full SHACL validation outcomes.
...
const CHAIN_SHAPES: &str = r#"
<urn:test:book-shape> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://www.w3.org/ns/shacl#NodeShape> .
<urn:test:book-shape> <http://www.w3.org/ns/shacl#targetClass> <urn:test:Book> .
<urn:test:book-shape> <http://www.w3.org/ns/shacl#property> <urn:test:author-property> .
<urn:test:author-property> <http://www.w3.org/ns/shacl#path> <urn:test:author> .
<urn:test:author-property> <http://www.w3.org/ns/shacl#property> <urn:test:name-property> .
<urn:test:name-property> <http://www.w3.org/ns/shacl#path> <urn:test:name> .
<urn:test:name-property> <http://www.w3.org/ns/shacl#minCount> "1"^^<http://www.w3.org/2001/XMLSchema#integer> .
"#;
...
#[test]
fn chain_delta_matches() {
    // The name check runs on author values, so each change must rerun the whole chain.
    let (_directory, node) = node();
    let data = GraphId::new("urn:test:chain-data");
    let shapes = GraphId::new("urn:test:chain-shapes");
    insert_shape_text(&node, &shapes, CHAIN_SHAPES);
    let book_type = |insert| {
        change(
            &data,
            insert,
            "urn:test:book",
    ...
    node.apply_changes_unchecked(&data, vec![book_type(true)])
        .unwrap();
    let schema = node
        .compile_shacl(&AUTH, &shapes, &ShaclCompileOptions::default())
        .unwrap();
    ...
    let steps = [
        ...
        (name(true), true),
        (name(false), false),
        (book_type(false), true),
        (book_type(true), false),
        (author(false), true),
    ];
    for (change, conforms) in steps {
        let changes = vec![change];
        let incremental = node
            .validate_shacl_delta(&AUTH, &data, &schema, &changes, &delta_options)
            .unwrap();
        ...
        let full = node
            .validate_shacl(&AUTH, &data, &schema, &options)
            .unwrap();
        assert_eq!(full.conforms, conforms);
        assert_eq!(incremental.conforms, conforms);
        assert_eq!(incremental.results, full.results);
    }
}
